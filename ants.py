...
class Ant(Insect):
    ...
    def bee(self):
        """bee this ants's damage, if it has not already been beed."""
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if self.bee_check == False:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        looking_at_place = self.place
        i = 0
        
        while not looking_at_place.is_hive:
            if len(looking_at_place.bees) > 0 and (self.lower_bound <= i <= self.upper_bound):
                return random_bee(looking_at_place.bees)
            i+=1
            looking_at_place = looking_at_place.entrance
        return None
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if amount >= self.health:
            damage_deal = self.damage + self.health
        else:
            damage_deal = amount 
        
        for each in copy_bees:
            each.reduce_health(damage_deal)
            
        self.place.bees = [bee for bee in copy_bees if bee.health > 0]
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if len(self.place.bees) >0: # if its place has a bee then eat it
...
class ContainerAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        if self.ant_contained != None: #if there is an Ant in the container
...
class TankAnt(ContainerAnt):
    ...
    def __init__(self):
        super().__init__(2)
    def action(self, gamestate):
        super().action(gamestate) # Ant's action
...
# END Problem 12
    ...
    def action(self, gamestate):
        """A queen ant throws a leaf, but also bees the damage of ants
        in her tunnel.
        """
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        ...
        while current != None:
            if current.ant != None:
                if isinstance(current.ant, ContainerAnt) and current.ant.ant_contained != None: #if it is container which contained an Ant
                    current.ant.ant_contained.bee()
                current.ant.bee()
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        dist = 0
        while current != None:
            if current.ant is not self and current.ant is not None: # add ants
                list_shot[current.ant] = dist
            for each in current.bees: # add bees
                list_shot[each] = dist
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        damage_deal = self.damage - (distance * 0.25) - self.insects_shot/16
        return damage_deal
        # END Problem Optional 2
